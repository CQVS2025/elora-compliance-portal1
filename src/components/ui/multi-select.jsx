...
/**
 * @typedef {{ value: string, label: string, disable?: boolean, fixed?: boolean, [key: string]: string | boolean | undefined }} Option
 */
...
function transToGroupOption(options, groupBy) {
  ...
  options.forEach((option) => {
    const key = (option[groupBy] && String(option[groupBy])) || '';
 ...
 * @param {Option[]} [props.defaultOptions]
 * @param {Option[]} [props.options]
 * @param {string} [props.placeholder]
 * @param {React.ReactNode} [props.loadingIndicator]
 * @param {React.ReactNode} [props.emptyIndicator]
 * @param {number} [props.delay]
 * @param {boolean} [props.triggerSearchOnFocus]
 * @param {(v: string) => Promise<Option[]>} [props.onSearch]
 * @param {(v: string) => Option[]} [props.onSearchSync]
 * @param {(options: Option[]) => void} [props.onChange]
 * @param {number} [props.maxSelected]
 * @param {(maxLimit: number) => void} [props.onMaxSelected]
 * @param {boolean} [props.hidePlaceholderWhenSelected]
 * @param {boolean} [props.disabled]
 * @param {string} [props.groupBy]
 * @param {string} [props.className]
 * @param {string} [props.badgeClassName]
 * @param {boolean} [props.selectFirstItem]
 * @param {boolean} [props.creatable]
 * @param {React.ComponentPropsWithoutRef<typeof Command>} [props.commandProps]
 * @param {Omit<React.ComponentPropsWithoutRef<typeof CommandPrimitive.Input>, 'value'|'placeholder'|'disabled'>} [props.inputProps]
 * @param {boolean} [props.hideClearAllButton]
 */
const MultipleSelector = ({
  ...
  useEffect(() => {
    ...
    if (triggerSearchOnFocus) void doSearch();
    if (debouncedSearchTerm) void doSearch();
  ...
  const commandFilter = React.useCallback(() => {
    ...
    if (creatable) {
      return (val, search) => (val.toLowerCase().includes(search.toLowerCase()) ? 1 : -1);
    }
    return undefined;
        ...
        <div className="flex flex-wrap gap-1">
          {selected.map((option) => (
            <div
              key={option.value}
              className={cn(
                'animate-fade-in bg-background text-secondary-foreground hover:bg-background relative inline-flex h-7 cursor-default items-center rounded-md border pr-7 pl-2 text-xs font-medium transition-all disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-[fixed]:pr-2',
                badgeClassName
              )}
            ...
            >
              ...
              <button
                type="button"
                className="text-muted-foreground/80 hover:text-foreground focus-visible:border-ring focus-visible:ring-ring/50 absolute -inset-y-px -right-px flex size-7 items-center justify-center rounded-r-md border border-transparent p-0 outline-none transition-[color,box-shadow] focus-visible:ring-[3px]"
                onKeyDown={(e) => {
