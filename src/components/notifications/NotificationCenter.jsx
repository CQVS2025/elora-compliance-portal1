...
export default function NotificationCenter() {
  ...
  const { data: notifications = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const allNotifications = await supabaseClient.tables.Notification.filter(
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async (notificationId) => {
      await supabaseClient.tables.Notification.update(notificationId, { read: true });
    },
    onSuccess: () => {
  ...
  const markAllAsReadMutation = useMutation({
    mutationFn: async () => {
      ...
      await Promise.all(unreadIds.map(id => 
        supabaseClient.tables.Notification.update(id, { read: true })
      ));
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['notifications']);
  ...
  const deleteNotificationMutation = useMutation({
    mutationFn: async (notificationId) => {
      await supabaseClient.tables.Notification.delete(notificationId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['notifications']);
    }
